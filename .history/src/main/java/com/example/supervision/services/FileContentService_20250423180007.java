     ...
     private String extractTextFromPptx(InputStream is) throws IOException {
        ...
        try (XMLSlideShow ppt = new XMLSlideShow(is)) {
            for (XSLFSlide slide : ppt.getSlides()) {
                ...
                // Texte des zones de texte dans les layouts
                if (slide.getSlideLayout() != null) {
                    for (XSLFShape layoutShape : slide.getSlideLayout().getShapes()) {
                        if (layoutShape instanceof XSLFTextShape) {
                            content.append(((XSLFTextShape) layoutShape).getText()).append("\n");
                        }
                    }
                }
            }
