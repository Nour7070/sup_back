...
public class SuperviseurService {
    ...
    public double getAverageCoursesPerTrainer() {
        long totalCourses = courseRepository.count(); // Nombre total de cours
        long totalTrainers = getFormateurCount(); // Nombre total de formateurs
    
        if (totalTrainers == 0) {
            return 0.0; // Pour éviter une division par zéro
        }
        
        return (double) totalCourses / totalTrainers; // Calcul de la moyenne
    }
