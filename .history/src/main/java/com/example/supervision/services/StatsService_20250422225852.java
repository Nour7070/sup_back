...
public Map<String, Object> getFormateurStats(Long formateurId) {
    ...
    List<Map<String, Object categoriesRaw = courseRepository.countByCategorie(formateurId);
    Map<String, Long> coursParCategorie = new HashMap<>();
    
    for (Object[] row : categoriesRaw) {
        coursParCategorie.put((String) row[0], (Long) row[1]);
    }
    
    stats.put("coursParCategorie", coursParCategorie);
