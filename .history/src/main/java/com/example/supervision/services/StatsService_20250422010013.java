...
public Map<String, Object> getFormateurStats(Long formateurId) {
    ...
    stats.put("coursPublies", approuves);
    stats.put("coursEnAttente", enAttente);
    stats.put("etudiantsAbonnes", FormateurSubscriptionRepositorysubscriptionRepository.countByFormateurId(formateurId));
...
public Map<String, Object> getApprenantStats(Long apprenantId) {
    ...
    stats.put("name", apprenant.getNom() + " " + apprenant.getPrenom());
    stats.put("email", apprenant.getEmail());
    stats.put("photo", apprenant.getPhotoUrl()); // Ajoutez ce champ si nécessaire
    stats.put("age", Period.between(apprenant.getDateNaissance(), LocalDate.now()).getYears());
    stats.put("dateIntegration", apprenant.getDateInscription().format(DateTimeFormatter.ofPattern("dd/MM/yyyy")));
    
    // Statistiques
    stats.put("coursSuivis", coursProgresRepository.countByApprenantId(apprenantId));
    stats.put("quizzReussis", sessionRepository.countPassedQuizzes(apprenantId));
    stats.put("quizzEchoues", sessionRepository.countFailedQuizzes(apprenantId));
    stats.put("chapitresSuivis", coursProgresRepository.countChaptersCompleted(apprenantId));
