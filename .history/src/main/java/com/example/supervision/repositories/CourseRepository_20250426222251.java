     ...
     @Query("SELECT c.domaine, COUNT(c), COUNT(CASE WHEN c.status = 'APPROVED' THEN 1 END) " +
      ...
      @Query("SELECT c.id, c.titre, c.domaine, COUNT(e), SIZE(c.chapters) " +
       ...
       "GROUP BY c.id, c.titre, c.domaine")
        ...
        @Query("SELECT DISTINCT c FROM Cours c LEFT JOIN FETCH c.chapters LEFT JOIN FETCH c.apprenants WHERE c.status = :status")
    List<Cours> findByStatusWithChaptersAndApprenants(@Param("status") CourseStatus status);
}