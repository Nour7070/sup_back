...
public interface CourseRepository extends JpaRepository<Cours, UUID> {
    ...
    @Query("SELECT MAX(c.publishedAt) FROM Course c WHERE c.formateurId = :formateurId AND c.status = 'APPROVED'")
    LocalDateTime findLastPublishedDate(@Param("formateurId") Long formateurId);

    @Query("SELECT c.categorie AS categorie, COUNT(c) AS count FROM Course c WHERE c.formateurId = :formateurId GROUP BY c.categorie")
    List<Object[]> countByCategorie(@Param("formateurId") Long formateurId);

    @Query("SELECT c.langue AS langue, COUNT(c) AS count FROM Course c WHERE c.formateurId = :formateurId GROUP BY c.langue")
    List<Object[]> countByLangue(@Param("formateurId") Long formateurId);
