...
public interface CourseRepository extends JpaRepository<Cours, UUID> {
    ...
    /*@Query("SELECT c FROM Course c LEFT JOIN FETCH c.chapters WHERE c.status = 'PENDING'")
    List<Cours> findByStatusWithChapters(CourseStatus status);*/

    @Query("SELECT c FROM Course c LEFT JOIN FETCH c.chapters WHERE c.status = :status")
List<Course> findByStatusWithChapters(@Param("status") CourseStatus status);
    ...
    List<Object[]> countByLangue(Long formateurId);
     ...
     @Query("SELECT c.domaine, COUNT(c), COUNT(CASE WHEN c.status = 'APPROVED' THEN 1 END) " +
      ...
      /*@Query("SELECT c.id, c.titre, c.domaine, COUNT(e), SIZE(c.chapters) " +
       "FROM Cours c JOIN c.apprenants e WHERE c.status = 'APPROVED' " +
       "GROUP BY c.id, c.titre, c.domaine")
        List<Object[]> findCoursesWithStudentCount();

        @Query(value = "SELECT EXISTS(SELECT 1 FROM apprenant_cours WHERE cours_id = :coursId AND apprenant_id = :apprenantId)", nativeQuery = true)
        boolean existsApprenantInscrit(@Param("coursId") UUID coursId, @Param("apprenantId") Long apprenantId);*/
