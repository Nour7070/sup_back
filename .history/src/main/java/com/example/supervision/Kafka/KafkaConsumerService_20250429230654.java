...
public void handleChapterCreated(String message) {
    try {
        ...
        logger.info("Chapitre {} créé pour le cours {}", chapter.getId(), courseId);

    } catch (JsonProcessingException e) {
        logger.error("Erreur de désérialisation du message Kafka: {}", message, e);
        // Vous pourriez aussi envoyer le message vers une Dead Letter Queue ici
    } catch (IllegalArgumentException e) {
        logger.error("Données du chapitre invalides: {}", e.getMessage());
    } catch (Exception e) {
        logger.error("Erreur inattendue lors du traitement du chapitre", e);
    }
}
}
