...
import com.example.supervision.models.Apprenant;
import com.example.supervision.models.Formateur;
import com.example.supervision.models.FormateurStatus;
...
public class KafkaConsumerService {
    ...
    public void handleFormateurPending(Map<String, Object> formateurData) {
        try {
            log.info("Réception d'un nouveau formateur en attente: {}", formateurData.get("email"));
            ...
            if (existingFormateur.isPresent()) {
                ...
                log.info("Mise à jour du formateur existant: {}", email);
            } else {
                formateur = new Formateur();
                formateur.setEmail(email);
                log.info("Création d'un nouveau formateur: {}", email);
            }
            
            // Mise à jour des données du formateur
            formateur.setPrenom(prenom);
            formateur.setNom(nom);
            formateur.setStatus(FormateurStatus.valueOf(status));
            formateur.setCertificatsUrls(certificatsUrls);
            formateur.setExperiencesUrls(experiencesUrls);
