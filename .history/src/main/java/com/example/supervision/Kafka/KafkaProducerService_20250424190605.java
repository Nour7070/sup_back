...
public class KafkaProducerService {
   ...
   public void sendModeratorData(Moderateur moderateur) {
        ...
        System.out.printl("Envoi des données du modérateur au service d'auth via Kafka: {}", moderateur.getEmail());
        
        kafkaTemplate.send(TOPIC, moderateur.getEmail(), moderateurData)
            .addCallback(
                result -> logger.info("Message envoyé avec succès pour : {}", moderateur.getEmail()),
