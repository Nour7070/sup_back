...
public void handleFormateurPending(String jsonPayload) {
    try {
        ...
        if (certificatsUrls != null) {
            for (String fileUrl : certificatsUrls) {
                // Extraction du nom de fichier uniquement
                String fileName = extractFileNameFromUrl(fileUrl);
                
                Document doc = new Document();
                doc.setTitle("Certificat - " + extractFileDisplayName(fileName));
                doc.setFileUrl(fileName); // Stockage du nom de fichier uniquement
                doc.setDocumentType("CERTIFICAT");
                doc.setFormateur(formateur);
                documents.add(doc);
                System.out.println("Ajout du certificat: " + fileName);
        ...
        // Traitement des expériences
        if (experiencesUrls != null) {
            for (String fileUrl : experiencesUrls) {
                // Extraction du nom de fichier uniquement
                String fileName = extractFileNameFromUrl(fileUrl);
...
// Méthode pour extraire uniquement le nom du fichier d'une URL
private String extractFileNameFromUrl(String fileUrl) {
    if (fileUrl == null || fileUrl.isEmpty()) {
        return "";
    }
    
    // Si c'est une URL avec un protocole (http://, https://, etc.)
    if (fileUrl.contains("://")) {
        try {
            ...
            // Pas une URL valide, on continue avec l'approche simple
        }
    }
    
    // Si c'est un chemin avec des séparateurs
    if (fileUrl.contains("/")) {
        return fileUrl.substring(fileUrl.lastIndexOf('/') + 1);
    }
    
    // Déjà un simple nom de fichier
    return fileUrl;
}

// Méthode pour extraire un nom d'affichage sans extension
private String extractFileDisplayName(String fileName) {
