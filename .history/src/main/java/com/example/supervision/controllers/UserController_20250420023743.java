...
public ResponseEntity<?> getUsersDirectFromDb(@PathVariable String userType) {
    ...
    @GetMapping("/courses/apprenants/{id}/courses")
public List<Course> getApprenantCourses(@PathVariable Long id) {
    ...
    List<InscriptionCours> inscriptions = inscriptionCoursRepository.findByApprenantId(id);
    
    // Extraire uniquement les cours de ces inscriptions
    return inscriptions.stream()
            .map(InscriptionCours::getCourse)
            .collect(Collectors.toList());
}
}
