...
public class ModerateurController {
    ...
    public ResponseEntity<Cours> approveCourse(@PathVariable UUID courseId) {
        ...
        String description = String.format("Moderator %s validate course %s .", moderateur.getNom() , course.getNom());
        activityLogService.log("MOD_APPROVE_COURSE", description, moderateur.getId());
...
public ResponseEntity<Cours> approveCourse(
    ...
    String description = String.format("Moderator %s %s approved course: %s", 
        moderateur.getPrenom(), moderateur.getNom(), cours.getTitre());
    
    activityLogService.log("COURSE_APPROVAL", description, moderateur.getId());
