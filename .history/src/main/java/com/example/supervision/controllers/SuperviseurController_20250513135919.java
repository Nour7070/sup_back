...
public class SuperviseurController {
    ...
    public ResponseEntity approveFormateurByEmail(@PathVariable String email ,  @PathVariable Long id) {
    ...
    public User rejectFormateurByEmail(@PathVariable String email , @PathVariable Long id) {
        User user = superviseurService.rejectFormateur(email);
        //kafkaProducerService.sendFormateurStatus(email, "REJECTED");
        kafkaProducerService.sendFormateurStatus(formateur.getId(), formateur.getEmail(), "REJECTED");
        return user;
    }
