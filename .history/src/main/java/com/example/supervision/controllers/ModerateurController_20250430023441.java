...
public ResponseEntity<Cours> approveCourse(@PathVariable UUID courseId) {
    ...
    // Log ici pour vérifier si la méthode est atteinte
    log.info("Tentative d'approbation du cours avec l'ID: " + courseId);
    
    cours.setStatus(CourseStatus.APPROVED);
    Cours savedCourse = courseRepository.save(cours);
    
    // Log ici aussi pour voir la mise à jour du cours
    log.info("Cours approuvé avec succès: " + savedCourse.getId());
    
    // Validation (commenté, vérifier s'il doit être utilisé)
    //validationResponseProducer.sendValidationResponse(courseId, true, "Cours approuvé");
