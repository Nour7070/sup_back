...
public ResponseEntity<Resource> downloadFileWithComplexName(HttpServletRequest request) {
    ...
    @GetMapping("/api/file-content/{fileName:.+}")
    public ResponseEntity<Resource> serveFileContent(@PathVariable String fileName) {
        try {
            Path filePath = Paths.get(uploadDir).resolve(fileName).normalize();
            Resource resource = new UrlResource(filePath.toUri());
    ...
    // Endpoint pour télécharger les fichiers (tous types)
    @GetMapping("/files/api/file-download/{fileName:.+}")
    public ResponseEntity<Resource> downloadFile(@PathVariable String fileName) {
        try {
            Path filePath = Paths.get(uploadDir).resolve(fileName).normalize();
            Resource resource = new UrlResource(filePath.toUri());
    ...
    // Endpoint fallback pour les URL complexes (route alternative)
